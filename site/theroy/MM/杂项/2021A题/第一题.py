...
# 筛选满足条件的主索节点
zuobiao_1 = [list(zuobiao[i]) for i in range(num_1) if float(zuobiao[i][1])**2 + float(zuobiao[i][2])**2 <= 150**2]
num_2 = len(zuobiao_1)
...
cost, pos = optimizer.optimize(objfun, iters=500)
...
# 保存DataFrame到CSV文件
df_results.to_csv(r'D:\桌面\updated_coordinates.csv', index=False, encoding='GB2312')

print("坐标增量和更新后的坐标已保存到文件 'updated_coordinates.csv'.")