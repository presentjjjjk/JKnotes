...
## 量子化学精确解
!!! Note
    首先要明确的是,我们这里说的量子化学精确解是建立在Hartree-Fock近似带来的物理图景上的,从数学上来说,因为HF近似会缺失对一部分电子相互作用的描述,所以我们需要多组态相互作用这种数学方法给他打补丁,如果求解的方法是DFT,那么根本不存在所谓组态相关的物理图景.

    在传统的化学理论中,我们用一个能量最低的组态作为主要组态来描述整个体系,例如,在分子轨道理论中,我们考虑电子总是优先成对占据能量较低的$\sigma$成键轨道,如果存在简并轨道,则根据洪特规则自旋平行的占据这些轨道,这样一种占据方法对应能量最低的状态.

    而量子化学理论告诉我们,精确解必须由完全组态相关描述,这是一种更加先进的物理图景,我们之所以更加相信多组态相关这个物理图景,是因为其精确描述的波函数带来更强的可解释性,包含更多的信息.
...
完全组态相关要求我们表示波函数的时候用上所有的组态,首先,要明晰组态的定义,组态和电子构型类似,都是描述电子状态的一个名词,只不过,电子构型只对电子的轨道排布做出要求($1s^22s^22p^1$),没有对电子的自旋做出描述,而电子组态对电子自旋也给予了描述,两者的定义类似.
...
对于某个电子,我们近似假设其与其他电子的相互作用是一种平均的,孤立的相互作用,也就是说,一个电子与其他电子的相互作用采取所谓的平均场近似,即有fock算符:

$$
\hat{f}=\hat{T}+\sum_{i,j}V_{Ne}+\widetilde{V}_{ee}(r)
$$

通过求解fock算符的本征方程,可以得到所谓的 **单电子轨道波函数** .

那么总波函数可以写成各个分子轨道波函数的乘积(联合分布等于独立分布的乘积):
...
然而,上述波函数不满足泡利规则----互换任意两个电子的标号,总波函数需要变号,所以上述波函数需要修正成slater行列式的形式.
...
总分子轨道可以分离变量成为N个单电子轨道,并且可以用N个单电子轨道的Slater行列式来表示,这是 **平均场近似的直接结果** .
...
选一组分子轨道,满足$\phi_m(x)$可以被这组分子轨道的其他轨道线性表示,这样就能化简成为我们之前看到的本征值问题,也就是fock算符可以对角化.
...
$$
h_{\mu\nu}=\langle\phi_\mu|\hat{h}|\phi_\nu\rangle=\int dx_1 \phi^*_\mu(x_1)(-\frac{1}{2}\nabla^2-\sum_A \frac{Z_A}{|r_1-R_A|})\phi_\nu(x_1)
$$

这个积分是好算的,所以我们认为上述积分是已知的.

$$
J_{\mu\nu}=\int\phi^*_\mu(x_2)\left[\sum_{i=1}^N \int dx_1 \frac{\psi_i^*(x_1)\psi_i(x_1)}{r_{12}} \right]\phi_\nu(x_2) dx_2
$$
...
$$
D_{\alpha\beta}=\sum_i c^*_{\alpha i} c_{\beta i}
$$
