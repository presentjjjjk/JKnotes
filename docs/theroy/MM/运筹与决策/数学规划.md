...
$$
z=\begin{cases} \vec{w}\cdot\vec{x_i}+b>0, &x_i \in C_1  \\\vec{w}\cdot\vec{x_i}+b<0, &x_i \in C_2  \end{cases}
$$
...
$$
\begin{aligned}
\max \min \quad&|\vec{w}\cdot\vec{x_i}+b| \\
s.t.\quad & y_i(\vec{w}\cdot\vec{x_i}+b)>0\\
&\vec{w}\cdot\vec{w}=1
\end{aligned}
$$
...
$$
\begin{aligned}
\max \min \quad&y_i(\vec{w}\cdot\vec{x_i}+b) \\
s.t.\quad 
&\vec{w}\cdot\vec{w}=1
\end{aligned} \tag{1}
$$
...
$$
\begin{aligned}
\min \quad &\vec{w}\cdot\vec{w} \\
s.t.\quad &y_i(\vec{w}\cdot\vec{x_i}+b)\ge 1
\end{aligned}\tag{2}
$$

$(2)$ 和 $(1)$ 的解存在如下关系:

若$(\vec{w_0},b_0)$是$(2)$的最优解,那么$(\frac{\vec{w_0}}{\sqrt{\Vert w_0\Vert}},\frac{b_0}{\sqrt{\Vert w_0\Vert}})$则是$(2)$的最优解.

!!! tip "证明"

      设$(\vec{w^*},b^*)$是$(2)$的最优解,那么就会有最小的最优值:

      $$
      \gamma^*=\min y_i(\vec{w^*}\cdot\vec{x_{i}}+b^*)
      $$

      所以对于任意的$i=1,2, \ldots n$,始终会有:

      $$
      y_i(\vec{w^*}\cdot\vec{x_{i}}+b^*)\ge \gamma^*
      $$

      于是就得到:

      $$
      y_i(\vec{w^*}/\gamma^*\cdot\vec{x_{i}}+b^*/\gamma^*)\ge 1
      $$

      所以 $(\vec{w^*}/\gamma^*,b^*/\gamma^*)$ 是$(2)$的一个可行解,又因为$(\vec{w_0},b_0)$是$(2)$的最优解,所以:

      $$
      \vec{w^*}/\gamma^*\cdot\vec{w^*}/\gamma^*=\frac{1}{(\gamma^*)^2}\ge  \vec{w_0}\cdot \vec{w_0}
      $$

      所以:

      $$
      y_i(\frac{\vec{w_0}}{\sqrt{\Vert w_0\Vert}}\cdot \vec{x_i}+\frac{b_0}{\sqrt{\Vert w_0\Vert}})\ge \gamma^*y_i(\vec{w_0}\cdot\vec{x_i}+b_0)\ge \gamma^*
      $$

      所以解 $(\frac{\vec{w_0}}{\sqrt{\Vert w_0\Vert}},\frac{b_0}{\sqrt{\Vert w_0\Vert}})$ 的目标值不会小于最小的最优值,所以它也是最优解.
