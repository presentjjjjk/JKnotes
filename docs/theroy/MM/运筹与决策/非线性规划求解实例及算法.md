...
Python库中的绝大多数求解函数都只能得到局部最优解,尤其是基于梯度的算法,会更快地陷入到局部最小值当中去,一个朴素的解决办法是,寻找尽可能多的不同的出发点,基于这些出发点去执行优化,然后比较不同的局部最小值.

还有一种能够得到全局最小值的办法就是启发式算法,启发式算法大多以跳出局部最小值为目标进行了精妙的设计,Py的一些库的某些函数就是基于启发式算法来帮助求解全局最小值的.

最常用的函数就是 SciPy库 的 minimize 函数,他允许接收非线性目标函数和非线性约束,并且支持多种求解方案.
